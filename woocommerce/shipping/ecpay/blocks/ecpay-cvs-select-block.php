...
class RY_ECPay_Shipping_Cvs_Select_Block implements IntegrationInterface
{
    ...
    public function initialize()
    {
        ...
        $script_asset = include RY_WTP_PLUGIN_DIR . 'assets/js/blocks/shipping/ecpay/cvs-select-map/index.asset.php';
        wp_register_script('ry-ecpay-cvs-select-map', RY_WTP_PLUGIN_URL . 'assets/js/blocks/shipping/ecpay/cvs-select-map/index.js', $script_asset['dependencies'], $script_asset['version'], true);
        wp_set_script_translations('ry-ecpay-cvs-select-map', 'ry-woocommerce-tools-pro', RY_WTP_PLUGIN_LANGUAGES_DIR);

        $script_asset = include RY_WTP_PLUGIN_DIR . 'assets/js/blocks/shipping/ecpay/cvs-select-map/frontend.asset.php';
        wp_register_script('ry-ecpay-cvs-select-map-frontend', RY_WTP_PLUGIN_URL . 'assets/js/blocks/shipping/ecpay/cvs-select-map/frontend.js', $script_asset['dependencies'], $script_asset['version'], true);
    ...
    public function get_script_data()
    {
        ...
        return [
            ...
            'cvsInfo' => [
                'name' => WC()->checkout->get_value('CVSStoreName'),
                'address' => WC()->checkout->get_value('CVSAddress'),
                'phone' => WC()->checkout->get_value('CVSTelephone')
            ]
        ];
    }
}
