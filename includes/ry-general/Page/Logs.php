...
final class Logs extends AbstractAdminPage
{
    ...
    {
        $menu_list[] = [
            'name' => __('Logs', 'ry-woocommerce-tools-pro'),
            'slug' => 'ry-logs',
    ...
    public function output_page(): void
    {
        ...
        echo '<div class="wrap">';
        echo '<h1 class="wp-heading">' . esc_html__('Logs', 'ry-woocommerce-tools-pro') . '</h1>';
        if ($current_group === '') {
            echo '<p>' . esc_html__('No logs found.', 'ry-woocommerce-tools-pro') . '</p>';
            echo '</div>';
            return;
        }
        ...
        if (isset($this->log_list[$current_group], $this->log_list[$current_group][$current_log])) {
            ...
            if (str_starts_with($current_file, $this->log_path)) {
                if (wp_delete_file($current_file)) {
                    $this->add_notice('success', __('Delete successful.', 'ry-woocommerce-tools-pro'));
                    $current_log = '';
                    ...
                    $this->add_notice('error', __('Delete failed.', 'ry-woocommerce-tools-pro'));
                }
